...
from utilities import (create_folder, get_filename, traverse_folder, 
    ...
    OnsetsFramesPostProcessor, HackkeyRegressionPostProcessor, HackkeyTargetProcessor)
import config
from inference import PianoTranscription

def calculate_hackkey_metrics(ref_hackkey, est_hackkey):
    """Calculate hackkey metrics.

    Args:
      ref_hackkey: np.array, shape (classes_num, segment_samples)
      est_hackkey: np.array, shape (classes_num, segment_samples)
    ...
    Returns:
      ...
      cosine: float
    """
    if ref_hackkey.shape[0] > est_hackkey.shape[0]:
        ref_hackkey = ref_hackkey[:est_hackkey.shape[0], :]
    elif ref_hackkey.shape[0] < est_hackkey.shape[0]:
        est_hackkey = est_hackkey[:ref_hackkey.shape[0], :]
    
    mae = np.mean(np.abs(ref_hackkey - est_hackkey))
    mse = np.mean((ref_hackkey - est_hackkey) ** 2)
    # corr = pearsonr(ref_hackkey.flatten(), est_hackkey.flatten())[0]
    dtwd = dtw(ref_hackkey.T, est_hackkey.T).distance / ref_hackkey.shape[0]
    # cosine = 1 - cosine(ref_hackkey.flatten(), est_hackkey.flatten())
...
def infer_prob(args):
    ...
    for n, hdf5_path in enumerate(hdf5_paths):
        with h5py.File(hdf5_path, 'r') as hf:
            if "Lesson" not in hdf5_path:
                continue
            
            if (hf.attrs['split'].decode() == split) and (hf.attrs['source'].decode() == 'Lesson'):
                print(n, hdf5_path)
                prob_path = os.path.join(probs_dir, '{}.pkl'.format(get_filename(hdf5_path) + f"_{n}"))
                if os.path.exists(path=prob_path):
                    continue
                n += 1
                ...
                target_processor = HackkeyTargetProcessor(
                    segment_seconds=len(audio) / sample_rate, 
                    frames_per_second=frames_per_second, begin_note=begin_note, 
                    classes_num=classes_num)
                ...
                hackkey_data = hf["hackkey"]
                pedal_data = None

                # Process hackkey to target
                (target_dict, hackkey_segment, notes) = \
                target_processor.process(0, hackkey_data,
                extend_pedal=False)

                ref_on_off_pairs = np.array([[note['onset'], note['offset']] for note in notes])
                ref_midi_notes = np.array([note['key'] for note in notes])
                ref_velocity = np.array([note['velocity'] for note in notes])
                ref_hackkey = [note['hackkey_values'] for note in notes]
                ...
                total_dict['ref_hackkey'] = ref_hackkey
                total_dict['ref_hackkey_segment'] = hackkey_segment
                total_dict['duration'] = len(audio) / sample_rate
...
class ScoreCalculator(object):
    ...
    def metrics(self, params):
        ...
        n = 0
        list_args = []
        for n, hdf5_path in tqdm(enumerate(self.hdf5_paths), total=len(self.hdf5_paths)):
            if "Lesson" not in hdf5_path:
                continue
            
            with h5py.File(hdf5_path, 'r') as hf:
                if (hf.attrs['split'].decode() == self.split) and (hf.attrs['source'].decode() == 'Lesson'):
    ...
    def calculate_score_per_song(self, args):
        ...
        prob_path = os.path.join(self.probs_dir, '{}.pkl'.format(get_filename(hdf5_path) + f"_{n}"))
        ...
        elif self.post_processor_type == 'regression_hackkey':
        ...
        if self.post_processor_type == 'regression_hackkey':
            ...
            (est_on_off_note_vels, est_pedal_on_offs) = \
                post_processor.output_dict_to_note_arrays(output_dict)
        ...
        if self.post_processor_type == 'regression_hackkey':
            duration = total_dict['duration']
            est_hackkey_roll = np.zeros((88, int(duration * 1000)))
            time_index = np.arange(0, duration, 0.001)[:int(duration * 1000)]
            est_on_offs = np.array([[i['onset_time'], i['offset_time']] for i in est_hackkey_events])
            est_vels = np.array([i['velocity'] for i in est_hackkey_events]) * self.velocity_scale
            est_midi_notes = np.array([i['midi_note'] for i in est_hackkey_events])

            for i in est_hackkey_events:
                start = int(i['onset_time'] * 1000)
                ...
                if target_len > 0:
                    # Create evenly spaced indices for interpolation
                    orig_indices = np.linspace(0, len(hackkey)-1, len(hackkey))
                    target_indices = np.linspace(0, len(hackkey)-1, target_len)
                    
                    # Interpolate hackkey values [Linear interpolation]
                    if len(target_indices) > len(orig_indices):
                        interpolated_hackkey = np.interp(target_indices, orig_indices, hackkey)
                    else:
                        # If the target length is smaller, just take the first few values
                        interpolated_hackkey = hackkey[:target_len]
                    
                    # Fill the roll with interpolated values
                    est_hackkey_roll[key, start:end] = interpolated_hackkey
        else:
        ...
        if self.hackkey:
            hackkey_mae, hackkey_mse, hackkey_dtwd = \
                calculate_hackkey_metrics(total_dict['ref_hackkey_segment'],
                    est_hackkey_roll.T)
                
            return_dict['hackkey_mae'] = hackkey_mae
            return_dict['hackkey_mse'] = hackkey_mse
            return_dict['hackkey_dtwd'] = hackkey_dtwd
            
            print(f'Hackkey MAE: {hackkey_mae:.3f}, MSE: {hackkey_mse:.3f}, DTWD: {hackkey_dtwd:.3f}')
...
if __name__ == '__main__':
    ...
    parser_infer_prob.add_argument('--dataset', type=str, required=True, choices=['maestro', 'maps', 'hackkey'])
    parser_infer_prob.add_argument('--split', type=str, required=True)
    parser_infer_prob.add_argument('--post_processor_type', type=str, default='regression_hackkey')
    ...
    parser_metrics.add_argument('--dataset', type=str, required=True, choices=['maestro', 'maps', 'hackkey'])
    parser_metrics.add_argument('--split', type=str, required=True)
    parser_metrics.add_argument('--post_processor_type', type=str, default='regression_hackkey')
