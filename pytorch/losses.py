...
def mae(output, target, mask):
    """Mean absolute error (MAE) with mask. The positions where mask=0 will be 
    deactivated when calculation MAE.
    """
    matrix = torch.abs(output - target)
    # mask = mask / 2 + 0.5
...
def regress_onset_offset_frame_velocity_bce(model, output_dict, target_dict):
    ...
    velocity_loss = bce(output_dict['velocity_output'], target_dict['velocity_roll'], target_dict['frame_roll'])
    total_loss = onset_loss + offset_loss + 0.5 * frame_loss + velocity_loss * 0.2
    return onset_loss, offset_loss, frame_loss, velocity_loss, total_loss

def regress_onset_offset_frame_hackkey_mae(model, output_dict, target_dict):
    """High-resolution piano note regression loss, including onset regression, 
    offset regression, velocity regression and frame-wise classification losses.
    """
    onset_loss = bce(output_dict['reg_onset_output'], target_dict['reg_onset_roll'], target_dict['mask_roll'])
    offset_loss = bce(output_dict['reg_offset_output'], target_dict['reg_offset_roll'], target_dict['mask_roll'])
    frame_loss = bce(output_dict['frame_output'], target_dict['frame_roll'], target_dict['mask_roll'])
    hackkey_loss = mae(output_dict['hackkey_output'], target_dict['hackkey_roll'], target_dict['frame_roll'])
    total_loss = onset_loss + offset_loss + frame_loss + hackkey_loss
    return onset_loss, offset_loss, frame_loss, hackkey_loss, total_loss

def regress_onset_offset_frame_velocity_hackkey(model, output_dict, target_dict):
    """High-resolution piano note regression loss, including onset regression, 
    offset regression, velocity regression and frame-wise classification losses.
    """
    onset_loss = bce(output_dict['reg_onset_output'], target_dict['reg_onset_roll'], target_dict['mask_roll'])
    offset_loss = bce(output_dict['reg_offset_output'], target_dict['reg_offset_roll'], target_dict['mask_roll'])
    frame_loss = bce(output_dict['frame_output'], target_dict['frame_roll'], target_dict['mask_roll'])
    velocity_loss = bce(output_dict['velocity_output'], target_dict['velocity_roll'] / 127, target_dict['onset_roll'])
    hackkey_loss = mae(output_dict['hackkey_output'], target_dict['hackkey_roll'], target_dict['frame_roll'])
    total_loss = onset_loss + offset_loss + frame_loss + velocity_loss + hackkey_loss * 0.2
    return onset_loss, offset_loss, frame_loss, velocity_loss, hackkey_loss, total_loss
...
def get_loss_func(loss_type):
    ...
    elif loss_type == 'regress_onset_offset_frame_velocity_mae':
        return regress_onset_offset_frame_velocity_mae

    elif loss_type == 'regress_onset_offset_frame_velocity_hackkey':
        return regress_onset_offset_frame_velocity_hackkey
