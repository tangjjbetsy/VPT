...
class AcousticModelCRnn8Dropout(nn.Module):
    ...
    def forward(self, input):
        ...
        (x, hidden) = self.gru(x)
        embedding = hidden.transpose(0, 1).flatten(1)  # (batch_size, 256)
        x = F.dropout(x, p=0.5, training=self.training, inplace=False)
        output = torch.sigmoid(self.fc(x))
        return output, embedding

#NOTE(Jingjing) Added Hackkey model with the same architecture as velocity model for hackkey regression
#We didn't change the model name in order to load the checkpoints for fine-tuning more conveniently. 
class Regress_onset_offset_frame_velocity_CRNN(nn.Module):
    ...
    def forward(self, input):
        ...
        frame_output, frame_hidden = self.frame_model(x)  # (batch_size, time_steps, classes_num)
        reg_onset_output, onset_hidden = self.reg_onset_model(x)  # (batch_size, time_steps, classes_num)
        reg_offset_output, offset_hidden = self.reg_offset_model(x)    # (batch_size, time_steps, classes_num)
        velocity_output, vel_hiddens = self.velocity_model(x)    # (batch_size, time_steps, classes_num)
        
        #NOTE(Jingjing)
        # frame_output = frame_output[:, :, 0:88]
        # reg_onset_output = reg_onset_output[:, :, 0:88]
        # reg_offset_output = reg_offset_output[:, :, 0:88]
        # velocity_output = velocity_output[:, :, 0:88]
        ...
        output_dict = {
            ...
            'hackkey_output': velocity_output,
            'hiddens': torch.stack([frame_hidden, onset_hidden, offset_hidden, vel_hiddens])}

        return output_dict


class Regress_onset_offset_frame_velocity_hackkey_CRNN(nn.Module):
    def __init__(self, frames_per_second, classes_num):
        super(Regress_onset_offset_frame_velocity_hackkey_CRNN, self).__init__()
        ...
        self.reg_offset_model = AcousticModelCRnn8Dropout(classes_num, midfeat, momentum)
        self.velocity_model = AcousticModelCRnn8Dropout(classes_num, midfeat, momentum)
        #NOTE(Jingjing) Added Hackkey model
        self.hackkey_model = AcousticModelCRnn8Dropout(classes_num, midfeat, momentum)
    ...
    def forward(self, input):
        ...
        #NOTE(Jingjing) Added Hackkey model
        hackkey_output = self.hackkey_model(x)    # (batch_size, time_steps, classes_num)
        ...
        output_dict = {
            ...
            'hackkey_output': hackkey_output}         #NOTE(Jingjing) Added Hackkey model
