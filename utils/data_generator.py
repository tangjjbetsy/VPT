...
from utilities import (create_folder, int16_to_float32, traverse_folder, 
    pad_truncate_sequence, TargetProcessor, HackkeyTargetProcessor, write_events_to_midi, 
    plot_waveform_midi_targets, plot_waveform_hackkey_targets)
import config
...
class HackkeyMidiDataset(object):
    def __init__(self, hdf5s_dir, segment_seconds, frames_per_second, 
        max_note_shift=0, augmentor=None):
        """This class takes the meta of an audio segment as input, and return 
        the waveform and targets of the audio segment. This class is used by 
...
class HackkeyDataset(object):
    def __init__(self, hdf5s_dir, segment_seconds, frames_per_second, 
        ...
        self.target_processor = HackkeyTargetProcessor(self.segment_seconds, 
            self.frames_per_second, self.begin_note, self.classes_num)
        """Used for processing MIDI events to target."""
    ...
    def __getitem__(self, meta):
        ...
        Returns:
          data_dict: {
            ...
            'frame_roll': (frames_num, classes_num), 
            'velocity_roll': (frames_num, classes_num), 
            'hackkey_roll': (frames_num, classes_num),
            'mask_roll':  (frames_num, classes_num), 
        """
        [hdf5_path, start_time] = meta
        ...
        with h5py.File(hdf5_path, 'r') as hf:
            ...
            (target_dict, hackkey_segment, notes) = \
                self.target_processor.process(start_time, hackkey_data, 
                extend_pedal=False, note_shift=note_shift)
        ...
        debugging = False
        if debugging:
            np.save(f"debug/debug_{start_time}.npy", data_dict)
            plot_waveform_hackkey_targets(data_dict, start_time)
            exit()
...
class Sampler(object):
    def __init__(self, hdf5s_dir, split, segment_seconds, hop_seconds, 
        ...
        hdf5_paths = [path for path in hdf5_paths if "sound" in path]
