...
from utilities import (create_folder, float32_to_int16, create_logging, 
    get_filename, read_metadata, read_hackkey_metadata, read_hackkey, read_pedal, 
    align_hackkey_wav_pedal_and_remove_silence, align_hackkey_wav_and_remove_silence, read_midi, read_maps_midi)
import config
...
def pack_hackkey_midi_dataset_to_hdf5(args):
    """Load & resample MAESTRO audio files, then write to hdf5 files.
    ...
    # Paths
    csv_path = os.path.join(dataset_dir, 'hackkey_meta_concert_clean_upload_realign.csv')
    waveform_hdf5s_dir = os.path.join(workspace, 'hdf5s', 'hackkey_midi')
    ...
    # Read meta dict
    meta_dict = read_hackkey_metadata(csv_path)

    audios_num = len(meta_dict['sound_file'])
    logging.info('Total audios number: {}'.format(audios_num))
    ...
    # Load & resample each audio file to a hdf5 file
    for n in range(audios_num):
        logging.info('{} {}'.format(n, meta_dict['hackkey_file'][n]))

        # Read midi
        try:
            midi_path = os.path.join(dataset_dir, meta_dict['hackkey_file'][n].replace(".csv", ".midi"))
            midi_dict = read_midi(midi_path)
        except:
            continue

        packed_hdf5_path = os.path.join(waveform_hdf5s_dir, '{}.h5'.format(
            os.path.splitext(meta_dict['sound_file'][n])[0]))
        
        # if os.path.isfile(packed_hdf5_path):
        #     logging.info('File {} exists, skip'.format(packed_hdf5_path))
        #     continue
        
         # Load audio
        audio_path = os.path.join(dataset_dir, meta_dict['sound_file'][n])
        (audio, _) = librosa.load(audio_path, sr=sample_rate, mono=True)
        audio = audio / np.max(np.abs(audio))
        
        if meta_dict['wav_diff'][n] > 0:
            audio = audio[int(meta_dict["wav_diff"][n] * sample_rate):]

        create_folder(os.path.dirname(packed_hdf5_path))

        with h5py.File(packed_hdf5_path, 'w') as hf:
            hf.attrs.create('source', data=meta_dict['source'][n].encode(), dtype='S100')
            hf.attrs.create('split', data=meta_dict['split'][n].encode(), dtype='S20')
            hf.attrs.create('hackkey_file', data=meta_dict['hackkey_file'][n].encode(), dtype='S100')
            hf.attrs.create('midi_file', data=meta_dict['hackkey_file'][n].replace(".csv", ".midi").encode(), dtype='S100')
            hf.attrs.create('pedal_file', data=meta_dict['pedal_file'][n].encode(), dtype='S100')
            hf.attrs.create('sound_file', data=meta_dict['sound_file'][n].encode(), dtype='S100')
            hf.attrs.create('duration', data=meta_dict['aligned_duration'][n], dtype=np.float32)
...
def pack_hackkey_dataset_to_hdf5(args):
    """HackKey is a piano dataset used for train hackkey transcription
    system (optional). 
    Load & resample HackKey audio files, then write to hdf5 files.
    Args:
      dataset_dir: str, directory of dataset
      workspace: str, directory of your workspace
    """
    ...
    # Arguments & parameters
    dataset_dir = args.dataset_dir #Hackkey_Sony/
    workspace = args.workspace

    sample_rate = config.sample_rate

    # Paths
    csv_path = os.path.join(dataset_dir, 'hackkey_meta_lesson_seg_classification.csv')
    # csv_path = os.path.join(dataset_dir, 'hackkey_meta_concert_clean_upload_realign.csv')
    waveform_hdf5s_dir = os.path.join(workspace, 'hdf5s', 'hackkey')
    ...
    audios_num = len(meta_dict['source'])
    logging.info('Total audios number: {}'.format(audios_num))
    ...
    for n in range(audios_num):
        ...
        meta_dict_n = {key: meta_dict[key][n] for key in meta_dict.keys()}
        # hackkey_data, audio, pedal_data = align_hackkey_wav_pedal_and_remove_silence(meta_dict_n, dataset_dir)
        hackkey_data, audio = align_hackkey_wav_and_remove_silence(meta_dict_n, dataset_dir)
        audio_dur = len(audio) / sample_rate

        if hackkey_data is None:
            logging.info('Hackkey data is None, skip')
            continue
        ...
        with h5py.File(packed_hdf5_path, 'w') as hf:
            ...
            # hf.attrs.create('pedal_file', data=meta_dict['pedal_file'][n].encode(), dtype='S100')
            hf.attrs.create('sound_file', data=meta_dict['sound_file'][n].encode(), dtype='S100')
            hf.attrs.create('duration', data=audio_dur, dtype=np.float32)
            
            hf.create_dataset(name='hackkey', data=hackkey_data.to_numpy(), dtype=np.float32)
            hf.create_dataset(name='waveform', data=float32_to_int16(audio), dtype=np.int16)
            # hf.create_dataset(name='pedal', data=pedal_data.to_numpy(), dtype=np.float32)
...
if __name__ == '__main__':
    ...
    parser_pack_hackkey = subparsers.add_parser('pack_hackkey_dataset_to_hdf5')
    parser_pack_hackkey.add_argument('--dataset_dir', type=str, required=True, help='Directory of dataset.')
    parser_pack_hackkey.add_argument('--workspace', type=str, required=True, help='Directory of your workspace.')
    
    parser_pack_hackkey_midi = subparsers.add_parser('pack_hackkey_midi_dataset_to_hdf5')
    parser_pack_hackkey_midi.add_argument('--dataset_dir', type=str, required=True, help='Directory of dataset.')
    parser_pack_hackkey_midi.add_argument('--workspace', type=str, required=True, help='Directory of your workspace.')
    ...
    elif args.mode == 'pack_hackkey_dataset_to_hdf5':
        pack_hackkey_dataset_to_hdf5(args)
    
    elif args.mode == 'pack_hackkey_midi_dataset_to_hdf5':
        pack_hackkey_midi_dataset_to_hdf5(args)
